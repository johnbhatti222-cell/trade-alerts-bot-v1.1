"""
The core sniper engine: combines structure, liquidity, zones, momentum,
and chart patterns into a single graded signal with entry/SL/TP1-3 and
a tiered confidence output. This is where "do not force trades" is
enforced - if confluence is weak, it returns a NO_TRADE result.
"""

from config import WEIGHTS, SL_ATR_BUFFER, RR_TP1, RR_TP2, RR_TP3
...
from tiering import determine_tier
...
def evaluate_pair(label: str, htf_df, ltf_df) -> dict:
    ...
    # --- Confluence scoring: 4 independent categories ---
    score = 0
    reasons = []
    ...
    # Confirmation: RSI and chart-pattern breaks often reflect the SAME
    # underlying price swing rather than independent evidence, so this
    # category credits whichever fired — never both — to avoid one real
    # event being double-counted as two separate confluence votes.
    if momentum["confirmed"] or pattern_check["confirmed"]:
        score += WEIGHTS["confirmation"]
        if momentum["confirmed"]:
            reasons.append(momentum["reason"])
        if pattern_check["confirmed"]:
            reasons.append(pattern_check["reason"])

    # --- Tiering: session-aware grade instead of a single binary cutoff ---
    current_time = ltf_df["datetime"].iloc[-1]
    session = get_session(current_time)
    tier = determine_tier(score, session)

    if tier is None:
        return {
            **NO_TRADE,
            "reason": f"{label}: confluence score {score}/100 in a '{session}' session — no tier reached. "
